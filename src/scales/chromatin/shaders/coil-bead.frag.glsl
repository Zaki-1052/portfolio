...
// fbm-wobbled so they read as wound material, not printed stripes. The two
// publication regions carry a subtle extra rim glow — the invitation loci.
...
void main() {
  ...
  float wobble = fbm(vObjPos * 2.4) * 0.45;
  ...
  float mottle = fbm(vObjPos * 3.1 + vec3(5.2)) * 0.5 + 0.5;
