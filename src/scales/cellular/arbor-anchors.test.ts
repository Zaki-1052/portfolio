...
import { BRANCH_ANCHORS, computeBranchAnchors } from './arbor-anchors';

const LIMBS = [0, 1, 2] as const;

describe('branch anchors', () => {
  it('is deterministic and matches the module constant', () => {
    expect(computeBranchAnchors()).toEqual(computeBranchAnchors());
    expect(BRANCH_ANCHORS).toEqual(computeBranchAnchors());
  });

  it('yields three spatially distinct anchors', () => {
    for (let a = 0; a < LIMBS.length; a++) {
      for (let b = a + 1; b < LIMBS.length; b++) {
        const pa = BRANCH_ANCHORS[LIMBS[a]!];
        const pb = BRANCH_ANCHORS[LIMBS[b]!];
