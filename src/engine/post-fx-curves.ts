...
// warm tissue register (depth 0: strong golden bloom, visible grain + vignette)
 ...
 * token (~0.055 at tissue) and reaches 0 by the code scale.
 */
export function postFxCurveFor(depth: number): PostFxCurve {
  const d = clamp(depth, 0, 1);
  const warm = 1 - d; // 1 at tissue → 0 at expression
  return {
    // Eased warm falloff so bloom stays lush through the tissue band.
